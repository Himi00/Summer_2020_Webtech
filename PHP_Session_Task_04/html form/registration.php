<?php 
	...
	if (isset($_POST['submit'])) {
			...
			$dob = $_POST['dob'];
		if (empty($name) || empty($uname) || empty($email) || empty($pass) || empty($confirmpass) || empty($gender)  || empty($dob)) {
			$mess = "Required All Field";
		}else{
				if ($pass == $confirmpass) {
				$sql = "INSERT INTO `registration`(`name`, `email`, `uname`, `password`, `gender`, `dob`) VALUES ('$name', '$email', '$uname', '$pass', '$gender', '$dob')";
				execute($sql);
				$messs = "Registered Successful";
			}else{
